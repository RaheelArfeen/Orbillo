...
import { FaFacebookF, FaDribbble, FaBehance, FaLinkedinIn } from "react-icons/fa";
...
const Header = () => {
    ...
    return (
        // Adjusted py-4 for mobile, py-9 for desktop
        <div className={`w-full z-50 transition-all duration-300 py-4 md:py-9 ${open && 'fixed top-0'}`}>

            <header className="max-w-[1380px] px-4 mx-auto flex justify-between items-center w-full relative z-50">
                <div className="relative z-50 w-32 h-10 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 md:gap-6 relative z-50">
                    ...
                    >
                        <button className="md:py-[14px] py-3 md:px-[28px] px-4 rounded-full border border-white outfit text-white hover:bg-[#C9FF90] hover:border-[#C9FF90] hover:text-[#17241F] transition duration-300 cursor-pointer text-xs md:text-base whitespace-nowrap">
                            Book A Free Strategy Call
                        </button>
                    </a>

                    <button onClick={handleOpen} className="relative z-50 flex items-center justify-center">
                        <span className={`block transition-colors duration-500 ${open ? 'rotate-90 text-[#17241F]' : 'text-white'}`}>
                            {/* Removed hardcoded width/height, used Tailwind classes w-8 h-8 (mobile) -> w-12 h-12 (desktop) */}
                            <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="cursor-pointer w-8 h-8 md:w-12 md:h-12">
            ...
            {/* Menu Overlay */}
            <section
                className={`fixed inset-0 bg-[#E5FFCA] h-screen w-full z-40 overflow-y-auto transition-transform duration-500 ease-in-out ${open ? 'translate-y-0' : '-translate-y-full'}`}
            >
                {/* Adjusted padding-top: pt-28 for mobile (clears header), pt-46 for desktop */}
                <div className="max-w-[1380px] w-full mx-auto px-4 h-full flex flex-col justify-end md:justify-center pt-28 md:pt-46 pb-10">
                    <div className="flex flex-col lg:flex-row justify-between items-end w-full gap-12 lg:gap-0">

                        {/* LEFT COLUMN: Text & Actions */}
                        <div className="w-full lg:w-1/2 flex flex-col space-y-6 md:space-y-8 lg:pr-20">
                            <p className="text-[#17241F] reloceta text-2xl md:text-3xl max-w-[480px]">
                                Tell us about your project, goals, and timeline our team will get back to you {''}
                                <span className="text-[#02403A] underline decoration-[#02403A] decoration-3 underline-offset-5">within 24 hours.</span>
                            </p>

                            <div className="flex flex-wrap items-center gap-4 md:gap-6">
                                <button className="bg-[#02403A] text-[#C9FF90] hover:bg-[#C9FF90] hover:text-[#02403A] transition duration-300 px-8 py-3 md:px-16 md:py-4 rounded-full outfit text-lg md:text-xl">
                                    Contact us
                                </button>
                                <a
                                    ...
                                    rel="noopener noreferrer"
                                >
                                    <button className='px-4 py-3 md:px-6 md:py-4 flex items-center gap-3 outfit text-lg md:text-xl cursor-pointer text-[#16332F] hover:bg-[#16332F]/10 rounded-full transition duration-300'>
                            ...
                            <div className="flex items-center gap-4 pt-4">
                                {[FaFacebookF, FaDribbble, FaBehance, FaLinkedinIn].map((Icon, idx) => (
                                    <a key={idx} href="#" className="w-10 h-10 border border-[#17241F] rounded-full flex items-center justify-center text-[#17241F] hover:bg-[#17241F] hover:text-[#E5FFCA] transition-all duration-300">
                                        <Icon size={18} />
                                    </a>
                                ))}
                            </div>
                        ...
                        <div className="w-full lg:w-1/3 flex flex-col items-start space-y-4 md:space-y-6">
                            <Link href="/" className="text-xl md:text-3xl text-[#17241F] hover:opacity-70 transition-opacity font-medium px-4">
                                Home
                            </Link>
                            <Link href="/about" className="text-xl md:text-3xl text-[#17241F] hover:opacity-70 transition-opacity font-medium px-4">
                                About us
                            </Link>
                            ...
                            <div className="w-full">
                                <button
                                    ...
                                    className="flex items-center justify-between w-full text-xl md:text-3xl text-[#16332F] font-medium group px-4"
                                >
                                    <span>Services</span>
                                    <ChevronsDown
                                ...
                                <div className={`grid transition-all duration-300 ease-in-out overflow-hidden ${servicesOpen ? 'grid-rows-[1fr] opacity-100 mt-4' : 'grid-rows-[0fr] opacity-0'}`}>
                                    <div className="overflow-hidden flex flex-col  border-[#16332F]/10">
                                        {['Branding & Identity', 'UI/UX Design', 'Wordpress Development', 'Social Media Design'].map((item, i) => (
                                            <Link
                                                key={i}
                                                href="#"
                                                className="py-3 text-sm md:text-lg text-[#16332F]/72 hover:text-[#16332F] border-b border-[#000000]/15 last:border-none transition-colors pl-4"
                                            >
                                                {item}
                                            </Link>
                                        ))}
                            ...
                            <Link href="/case-studies" className="text-xl md:text-3xl text-[#16332F] hover:opacity-70 transition-opacity font-medium px-4">
                                Case Studies
                            </Link>
                        </div>
