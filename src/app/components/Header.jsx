    ...
    return (
        <div className={`w-full z-50 transition-all duration-300 py-4 md:py-6 lg:py-9 ${open ? 'fixed top-0' : 'sticky top-0'}`}>
            ...
            <header className="max-w-[1380px] px-4 mx-auto flex justify-between items-center w-full relative z-50">
                ...
                {/* 2. Desktop Navigation (Hidden on mobile, visible on LG) */}
                <nav className={`hidden lg:flex items-center gap-8 transition-opacity duration-300 ${open ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                    <Link href="/" className="text-white hover:text-[#C9FF90] transition-colors font-medium text-lg">
                        Home
                    </Link>
                    <Link href="/about" className="text-white hover:text-[#C9FF90] transition-colors font-medium text-lg">
                        About us
                    </Link>

                    {/* Desktop Dropdown */}
                    <div className="relative group">
                        <button className="flex items-center gap-1 text-white hover:text-[#C9FF90] transition-colors font-medium text-lg py-2">
                            Services
                            <ChevronsDown size={16} className="group-hover:rotate-180 transition-transform duration-300" />
                        </button>

                        {/* Dropdown Content */}
                        <div className="absolute top-8 left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                            <div className="bg-[#E5FFCA] text-[#17241F] rounded-2xl p-4 w-64 shadow-xl flex flex-col gap-2">
                                {SERVICES_LIST.map((item, i) => (
                                    <Link
                                        key={i}
                                        href="#"
                                        className="px-4 py-2 hover:bg-[#16332F]/10 rounded-lg transition-colors text-base font-medium"
                                    >
                                        {item}
                                    </Link>
                                ))}
                    ...
                    <Link href="/contact" className="text-white hover:text-[#C9FF90] transition-colors font-medium text-lg">
                        Contact Us
                    </Link>
                </nav>
                ...
                <div className="flex items-center gap-3 md:gap-6 relative z-50">
                    ...
                    {/* Hamburger Menu (Hidden on Desktop LG+, Visible on Mobile) */}
                    <button onClick={handleOpen} className="relative z-50 flex lg:hidden items-center justify-center p-1">
